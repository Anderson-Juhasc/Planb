    ...
    while 1:
        if (QR_code != ''):
            ...
            if version is None:
                ...
                infoconfimaddy = pubnub.publish({'channel' : 'atm','message' : {'some_text' : QR_code}})
                print infoconfimaddy
            QR_code = ''
        time.sleep(0.1)
        #pass
...
#bill 7
    ...
    if(billCredit != 0):
        ...
        if(ord(out[3]) & 0x10):
            infoconfimtend = pubnub.publish({'channel' : 'atm','message' : {'tenderd' : billCredit}})
            print infoconfirmtend
    time.sleep(0.1)
